...
def argo_gdac(save_to, lat_range=None,lon_range=None,start_date=None,end_date=None,sensors=None,floats=None,
  ...
  gdac_index = pd.read_csv(save_to + index_filename, delimiter=',', header=8)
  gdac_index['date'] = pd.to_datetime(gdac_index['date'], format='%Y%m%d%H%M%S', errors='coerce')
  gdac_index['date_update'] = pd.to_datetime(gdac_index['date_update'], format='%Y%m%d%H%M%S', errors='coerce')
